...
/**
 * fixTranscriptionSpacing: intelligently add spaces between words in accumulated transcription.
 */
const fixTranscriptionSpacing = (text) => {
  if (!text || typeof text !== 'string') return text;
  let result = normalizeWhitespace(text);
  result = result.replace(/([a-z])([A-Z])/g, '$1 $2');
  // Add space after punctuation followed by uppercase
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (nextWord && /[a-zA-Z]$/.test(word) && /^[a-zA-Z]/.test(nextWord)) {
      if (word.length <= 3 && nextWord.length > 3) {
        fixedWords.push(' ');
      }
      else if (word.length > 1 && nextWord.length > 1) {
        const compoundPatterns = /^(and|or|but|the|is|are|was|were|has|have|had|do|does|did|will|would|can|could|should|may|might|must|shall|will|can|may|shall|ought|need|dare)$/i;
        if (!compoundPatterns.test(word.toLowerCase())) {
          fixedWords.push(' ');
        }
      }
    }
...
const Aiv3chat = ({ open, onClose, medicalData: propMedicalData }) => {
  ...
  const saveMessageToHistory = async (message, role, timestamp) => {
    try {
      await apiService.post('/chat-history', {
        message,
        role,
    ...
    const loadApprovedMedicalData = () => {
      try {
        ...
        if (approvedRecords.length > 0) {
          const buildContextString = (records) => {
          ...
          const fullContext = buildContextString(approvedRecords);
          if (fullContext.trim()) {
  ...
  // Helper to accumulate bot messages (for streaming replies) - CORRECTED
  const accumulateBotMessage = useCallback((incomingText) => {
    setMessages(prev => {
        const lastIndex = prev.length - 1;
        const lastMessage = prev[lastIndex];

        // Ensure incomingText is trimmed and clean
        const newChunk = String(incomingText).trim();
        ...
        if (lastMessage && (lastMessage.role === 'assistant' || lastMessage.type === 'assistant')) {
            ...
            // Heuristic to decide if a space is needed
            let needsSpace = false;
            if (lastContent.length > 0) {
                const lastChar = lastContent[lastContent.length - 1];
                const firstChar = newChunk[0];

                // Add space if:
                // 1. The last character is a letter/number
                // 2. AND the first character of the new chunk is a letter/number
                // 3. AND the previous content does not end with punctuation that acts as a boundary.
            ...
            const accumulated = lastContent + separator + newChunk;
        ...
        // If not a continuation, start a new message (no leading space)
        return [...prev, {
            id: Date.now(),
            role: 'assistant',
            type: 'assistant',
            content: newChunk, // Use newChunk directly
            text: newChunk,
            timestamp: new Date()
        }];
    });
    ...
    serviceMessages.forEach((msg) => {
      ...
      // 2. Handle Bot Response (accumulate chunks and save only on turn completion)
           ...
           if (newChunk) {
               ...
               // Accumulate the chunk in the buffer for later saving
               assistantMessageBuffer.current += (assistantMessageBuffer.current ? ' ' : '') + newChunk;
